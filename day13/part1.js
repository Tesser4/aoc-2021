...
const countDots = (paper) => paper
  .map(line => line.reduce((a, c) => c === dot ? a + 1 : a, 0))
  .reduce((a, c) => a + c)
...
const solvePart1 = (fileData) => {
  ...
  const paper = setupPaper(coords)
  const [firstFold] = folds
  const [foldDirection, foldPosition] = firstFold
  let foldedPaper
  if (foldDirection === 'y')
    foldedPaper = foldUp(paper, foldPosition)
  else if (foldDirection === 'x')
    foldedPaper = foldLeft(paper, foldPosition)
